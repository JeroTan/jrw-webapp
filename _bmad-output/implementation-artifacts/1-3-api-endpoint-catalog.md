...
| `POST /api/auth/sessions`<br>`DELETE /api/auth/sessions/current`<br>`GET /api/auth/session`<br>`POST /api/customers`<br>`POST /api/email-verifications`<br>`POST /api/password-resets`<br>`POST /api/oauth/google/sessions` | auth | Stories 1.7, 1.8, 1.9, 1.10 | public or required by endpoint | `SUPER_ADMIN`, `ADMIN`, `CUSTOMER`, `PROSPECT` as applicable | `auth-password`, `email-token`, or TBD | TBD in owning story | `VALIDATION_FAILED`, `AUTH_REQUIRED`, `AUTH_FORBIDDEN`, `RESOURCE_NOT_FOUND`, `CONFLICT_STATE`, `RATE_LIMITED`, `INTERNAL_ERROR` as applicable | planned | Covers session login/logout/me, customer registration, email verification, password reset, and customer Google sign-in. Paths are planned contract anchors and may be refined by owning stories. |
