...
| `POST /api/password-resets`<br>`POST /api/oauth/google/sessions` | auth | Stories 1.9, 1.10 | public or required by endpoint | `CUSTOMER`, `PROSPECT` as applicable | `email-token` or TBD | TBD in owning story | `VALIDATION_FAILED`, `AUTH_REQUIRED`, `AUTH_FORBIDDEN`, `RESOURCE_NOT_FOUND`, `CONFLICT_STATE`, `RATE_LIMITED`, `INTERNAL_ERROR` as applicable | planned | Password reset and Google sign-in remain planned contract anchors. |
