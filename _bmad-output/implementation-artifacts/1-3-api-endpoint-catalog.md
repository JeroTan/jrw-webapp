...
| `POST /api/oauth/google/sessions` | auth | Story 1.10 | public or required by endpoint | `CUSTOMER`, `PROSPECT` as applicable | TBD | TBD in owning story | `VALIDATION_FAILED`, `AUTH_REQUIRED`, `AUTH_FORBIDDEN`, `RESOURCE_NOT_FOUND`, `CONFLICT_STATE`, `RATE_LIMITED`, `INTERNAL_ERROR` as applicable | planned | Google sign-in remains planned contract anchor. |
