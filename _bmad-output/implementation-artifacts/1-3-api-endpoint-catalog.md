...
| `POST /api/auth/sessions`<br>`DELETE /api/auth/sessions/current`<br>`GET /api/auth/session` | auth | Story 1.7 | `POST`: public<br>`DELETE`: optional<br>`GET`: optional | `POST`: `PROSPECT`<br>`DELETE`/`GET`: `PROSPECT`, `CUSTOMER`, `ADMIN`, `SUPER_ADMIN` | `POST`/`DELETE`: `auth-password`<br>`GET`: `public-read` | `tboxSignInBody`, `tboxSignInData`, `tboxSignOutData`, `tboxSessionInspectionData` in `src/server/routes/auth.routes.ts` | `VALIDATION_FAILED`, `AUTHENTICATION`, `ACCOUNT_SUSPENDED`, `EMAIL_NOT_VERIFIED`, `ADMIN_APPROVAL_REQUIRED`, `RATE_LIMITED`, `AUTH_REQUIRED`, `INTERNAL_ERROR` | complete | Session cookie is opaque `jrw_session`, HttpOnly, SameSite=Lax, Secure outside local HTTP; response bodies never return raw token. `DELETE` is idempotent when no active session exists. |
| `POST /api/customers`<br>`POST /api/email-verifications`<br>`POST /api/password-resets`<br>`POST /api/oauth/google/sessions` | auth | Stories 1.8, 1.9, 1.10 | public or required by endpoint | `CUSTOMER`, `PROSPECT` as applicable | `email-token` or TBD | TBD in owning story | `VALIDATION_FAILED`, `AUTH_REQUIRED`, `AUTH_FORBIDDEN`, `RESOURCE_NOT_FOUND`, `CONFLICT_STATE`, `RATE_LIMITED`, `INTERNAL_ERROR` as applicable | planned | Customer registration, email verification, password reset, and Google sign-in remain planned contract anchors. |
