...
| Migration files | `migrations/0000_classy_menace.sql` through `migrations/0006_striped_lord_hawal.sql` | Seven migration entries tracked. |
| Migration journal | `migrations/meta/_journal.json` | Version `7`, dialect `sqlite`, entries idx `0` through `6`. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `admins` | Exists with `email`, `password_hash`, `is_owner`, timestamps. | Stories 1.6, 1.11, 1.13 | Super Admin/Admin identity, approval, suspension, ownership transfer. | Audit logs, future sessions, ownership transfer records. | Epic 1. | Medium: unique owner invariant and owner credential seed changes. | Record seed/migration command, target D1, migration/seed output, and owner invariant verification. | Review unique Super Admin plan, credential handling, and rollback path. |
