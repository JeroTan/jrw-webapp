...
| Migration files | `migrations/0000_classy_menace.sql` through `migrations/0007_stormy_nighthawk.sql` | Eight migration entries tracked. Story 1.6 adds owner-only partial unique index. |
| Migration journal | `migrations/meta/_journal.json` | Version `7`, dialect `sqlite`, entries idx `0` through `7`. |
