...
| Migration files | `migrations/0000_classy_menace.sql` through `migrations/0013_wakeful_crystal.sql` | Fourteen migration entries tracked. Story 1.10 adds OAuth state tokens and provider lookup indexes after Story 1.9 migration `0012_public_morlocks.sql`. |
| Migration journal | `migrations/meta/_journal.json` | Version `7`, dialect `sqlite`, entries idx `0` through `13`. |
