...
| Migration files | `migrations/0000_classy_menace.sql` through `migrations/0016_products_brand_id_fk.sql` | Seventeen SQL migration files exist. Story 2 brand tables (`0015`) and post-retro product brand FK (`0016`) are manual SQL files pending remote apply evidence. |
| Migration journal | `migrations/meta/_journal.json` | Version `7`, dialect `sqlite`, entries idx `0` through `14`; manual SQL files `0015` and `0016` are not journaled snapshots. |
...
| Timestamp | Command | Target | Migration | SQL review | Remote development evidence |
| --- | --- | --- | --- | --- | --- |
| 2026-05-19T07:50+08:00 | Manual SQL written after schema update; no remote migration run. | Local manual SQL for development D1 schema | `migrations/0016_products_brand_id_fk.sql` | Adds nullable `products.brand_id` FK to `brands(id)` with `ON DELETE SET NULL`, backfills from legacy `products.brand` by id/name/slug, and creates `idx_products_brand_id`. No table rebuild or destructive drop. Requires `0015_funny_outlaw.sql` brand tables to be applied first. | Not applied remotely during implementation. Remote apply remains pending with `npm run db:migrate:remote` against `jrw-database-development` / `beabfd98-8611-4d58-8f1b-7a972b8af1ed`; MR. JRW owns final apply/build-test gate. |
...
| Brand tables | Exist in manual migration `0015_funny_outlaw.sql`; remote apply evidence still pending. | Stories 2.1-2.5 | Brand catalog groups and collaboration membership. | Admins, products, future brand membership history. | Epic 2. | Medium: product ownership refactor from plain text brand. | Record migration command, target D1, backfill strategy from `products.brand`, and success output. | Confirm brand is not tenant/store/merchant; review membership access scope. |
